...
public class Player3Controller2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown(PC2D.Input.JUMP3))
        {
            _motor.Jump();
            _motor.DisableRestrictedArea();
        }

        _motor.jumpingHeld = Input.GetButton(PC2D.Input.JUMP3);

        // XY freedom movement
        if (_motor.motorState == PlatformerMotor2D.MotorState.FreedomState)
        {
            _motor.normalizedXMovement = Input.GetAxis(PC2D.Input.HORIZONTAL3);
            _motor.normalizedYMovement = Input.GetAxis(PC2D.Input.VERTICAL3);

            return; // do nothing more
        }

        // X axis movement
        if (Mathf.Abs(Input.GetAxis(PC2D.Input.HORIZONTAL3)) > PC2D.Globals.INPUT_THRESHOLD)
        {
            _motor.normalizedXMovement = Input.GetAxis(PC2D.Input.HORIZONTAL3);
        }
        else
        ...
        if (Input.GetAxis(PC2D.Input.VERTICAL3) != 0)
        {
            bool up_pressed = Input.GetAxis(PC2D.Input.VERTICAL3) > 0;
