...
public class Player4Controller2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown(PC2D.Input.JUMP4))
        {
            _motor.Jump();
            _motor.DisableRestrictedArea();
        }

        _motor.jumpingHeld = Input.GetButton(PC2D.Input.JUMP4);

        // XY freedom movement
        if (_motor.motorState == PlatformerMotor2D.MotorState.FreedomState)
        {
            _motor.normalizedXMovement = Input.GetAxis(PC2D.Input.HORIZONTAL4);
            _motor.normalizedYMovement = Input.GetAxis(PC2D.Input.VERTICAL4);

            return; // do nothing more
        }

        // X axis movement
        if (Mathf.Abs(Input.GetAxis(PC2D.Input.HORIZONTAL4)) > PC2D.Globals.INPUT_THRESHOLD)
        {
            _motor.normalizedXMovement = Input.GetAxis(PC2D.Input.HORIZONTAL4);
        }
        else
        ...
        if (Input.GetAxis(PC2D.Input.VERTICAL4) != 0)
        {
            bool up_pressed = Input.GetAxis(PC2D.Input.VERTICAL4) > 0;
