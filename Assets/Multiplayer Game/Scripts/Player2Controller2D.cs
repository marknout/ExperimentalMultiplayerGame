...
public class Player2Controller2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_motor.motorState == PlatformerMotor2D.MotorState.FreedomState)
        {
            _motor.normalizedXMovement = Input.GetAxis(PC2D.Input.HORIZONTAL2);
            _motor.normalizedYMovement = Input.GetAxis(PC2D.Input.VERTICAL2);

            return; // do nothing more
        }

        // X axis movement
        if (Mathf.Abs(Input.GetAxis(PC2D.Input.HORIZONTAL2)) > PC2D.Globals.INPUT_THRESHOLD)
        {
            _motor.normalizedXMovement = Input.GetAxis(PC2D.Input.HORIZONTAL2);
        }
        else
        ...
        if (Input.GetAxis(PC2D.Input.VERTICAL2) != 0)
        {
            bool up_pressed = Input.GetAxis(PC2D.Input.VERTICAL2) > 0;
        ...
        else if (Input.GetAxis(PC2D.Input.VERTICAL2) < -PC2D.Globals.FAST_FALL_THRESHOLD)
        {
